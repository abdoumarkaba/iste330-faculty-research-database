...
public class FacultyPanel extends JFrame {
    ...
    public FacultyPanel(int facultyId) {
        ...
        JButton viewAbstractsBtn = new JButton("View My Abstracts");
        btnPanel.add(insertBtn); btnPanel.add(updateBtn); btnPanel.add(deleteBtn); btnPanel.add(viewAbstractsBtn);
        formPanel.add(btnPanel);

        // My Keywords display
        formPanel.add(new JLabel("My Keywords:"));
        myKeywordsArea = new JTextArea(2, 20);
        myKeywordsArea.setEditable(false);
        formPanel.add(new JScrollPane(myKeywordsArea));
        ...
        // Show faculty keywords on open (persistent display)
        List<String> kws = dao.getFacultyKeywords(facultyId);
        myKeywordsArea.setText(String.join(", ", kws));
        ...
        // Add Find Matching Students button to search panel
        JButton matchBtn = new JButton("Find Matching Students");
        searchPanel.add(matchBtn);
        ...
        // Find Matching Students - compares faculty keywords/abstracts to student keywords
        matchBtn.addActionListener(e -> {
            List<String> results = dao.searchStudentsByFacultyMatch(facultyId);
            tableModel.setRowCount(0);
            for (String r : results) {
                String[] parts = r.split(" - ");
                tableModel.addRow(new Object[]{parts[0], parts.length > 1 ? parts[1] : ""});
            }
            if (results.isEmpty()) JOptionPane.showMessageDialog(this, "No matching students found based on your keywords/abstracts.");
            else JOptionPane.showMessageDialog(this, "Found " + results.size() + " student(s) matching your interests!");
        });

        // View My Abstracts
        viewAbstractsBtn.addActionListener(e -> {
            List<String> abstracts = dao.getFacultyAbstracts(facultyId);
            if (abstracts.isEmpty()) {
                JOptionPane.showMessageDialog(this, "You have no abstracts.");
            } else {
                JTextArea area = new JTextArea(10, 50);
                area.setEditable(false);
                area.setText(String.join("\n\n", abstracts));
                JScrollPane scroll = new JScrollPane(area);
                JOptionPane.showMessageDialog(this, scroll, "My Abstracts", JOptionPane.PLAIN_MESSAGE);
            }
        });
    }
}
